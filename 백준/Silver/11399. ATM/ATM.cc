/*
    ...
    4번 사람은 3+1+4+3 = 11분, 5번 사람은 3+1+4+3+2 = 13분이 걸리게 된다. 이 경우에 각 사람이 돈을 인출하는데 필요한 시간의 합은 
    3+4+8+11+13 = 39분이 된다.
    줄을 [2, 5, 1, 4, 3] 순서로 줄을 서면, 2번 사람 은 1분만에, 5번 사람은 1+2 = 3분, 1번 사람은 1+2+3 = 6분, 4번 사람은 1+2+3+3 = 9분, 
...
int main()
{
    ...
    for(int i=0;i<N;i++)
        scanf("%d",&ATM[i]);
    qsort(ATM,N,sizeof(ATM[0]),compare);
    out = ATM[0];
    for(int i=1;i<N;i++)
    {
        ATM[i] = ATM[i] + ATM[i-1];
