...
def find_all_existing_problems() -> List[Problem]:
    """기존 README.md 파일에서 문제 정보를 추출"""
    problems = []
    try:
        if os.path.exists("README.md"):
            with open("README.md", 'r', encoding='utf-8') as f:
                ...
                for row in table_rows:
                    # 헤더 행과 구분선 행 건너뛰기
                    if '-|-' in row.group(0) or 'Level' in row.group(1):
                        continue
                    
                    level = row.group(1).strip()
                    number = row.group(2).strip()
                    name = row.group(3).strip()
                    path = re.search(r'\((.*?)\)', row.group(4)).group(1) if re.search(r'\((.*?)\)', row.group(4)) else ""
                    categories_html = row.group(5).strip()
                    
                    # HTML select에서 카테고리 추출
                    categories = []
                    if categories_html:
                        category_matches = re.finditer(r'<option>(.*?)</option>', categories_html)
...
def main():
    # 기존 README.md 파일에서 문제 정보 추출
    existing_problems = find_all_existing_problems()
    
    # 변경된 파일에서 새로운 문제 정보 추출
